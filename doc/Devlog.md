...
## 2026-03-04 — 首次真机测试 + 手部跟踪抖动修复（多轮迭代）

**里程碑：** 从今天起开始在 UFactory 850 真机上测试，之前所有开发和测试都在 Docker 模拟器上进行。

**发现的问题：手部跟踪抖动导致机械臂一顿一顿，桌子都在震**
- 现象：手部跟踪模式下整个桌子持续震动，尤其在手慢速移动或静止时
- 根因分析（5 层叠加）：
  1. MediaPipe 手腕检测帧间噪声 → 2× 水平放大 + 800mm Y/650mm Z 映射 → 每帧 7-16mm 原始位置跳变
  2. 没有死区 → 每帧都发新 servo 指令，即使只差 0.1mm
  3. 检测丢帧时清空滤波器 → 恢复检测后无平滑的位置跳变
  4. TRACK 速度 300mm/s → 12mm/帧余量，完全能跟踪噪声
  5. 跟踪时叠加了呼吸振荡（3mm+5mm 正弦波），慢速时变成抖动

**修复迭代：**

迭代 1 — 简单 EMA + 死区（部分有效）：
- EMA α=0.85 只减小了噪声振幅，但保留了锯齿波形，残余抖动仍可见
- 2mm 硬死区在 TRACK 模式有效，但误伤了 CALM 等模式（正弦波每帧步长 ~1.4mm < 2mm 死区，导致卡顿）

迭代 2 — One-Euro 自适应滤波器（最终方案，效果显著）：
- **用 1€ Filter（Casiez et al., CHI 2012）替换简单 EMA**
- 原理：根据输入速度自适应调节平滑强度
  - 手静止时：cutoff 降到 min_cutoff=1.0 Hz，极重平滑，抖动完全消除
  - 手快速移动：cutoff = min_cutoff + β×|速度|，自动放开跟踪
- 参数：位置 min_cutoff=1.0, beta=0.007；RPY min_cutoff=0.8, beta=0.004
- 死区改为仅 TRACK 模式启用（通过 `send_servo(deadzone=2.0)` 参数控制），数学生成的平滑运动不受影响
- 检测丢失 0.5s 宽限期：短暂丢帧保持最后已知位置，不重置滤波器
- 移除 TRACK 模式呼吸叠加，降速 300→200 mm/s

**经验教训：**
- EMA 对实时交互输入不够用——固定系数无法同时满足"静止时平滑"和"移动时响应"
- 1€ Filter 是 noisy real-time tracking 的业界标准方案，专为此场景设计
- 死区不能全局启用——数学生成的慢速运动步长可能低于阈值，造成卡顿
- 模拟器上看不出任何问题——真机的机械惯性和结构振动会放大一切微小噪声
